...
public class StockerDao {
	...
	public static int creerStock (Stocker unStock) {
		...
		int resultat = ConnexionMySql.execReqMaj("insert into STOCK values ("+unStock.getQteStock()+",'"+unVisiteur.getMatricule()+"','"+unMedicament.getDepotLegal()+"')");
		return resultat;
	}
	...
	public static ArrayList<Stocker> retournerCollectionStockVisiteur(String unMatricule) {
		...
		ResultSet reqSelection = ConnexionMySql.execReqSelection("select * from STOCK where MATRICULE ='"+unMatricule+"'");
			try {
	...
	public static boolean checkIfStockExist(String matricule, String medicamentDepotLegal) {
		...
		ResultSet reqSelection = ConnexionMySql.execReqSelection("SELECT * FROM stock WHERE MATRICULE = '"+matricule+"' AND MED_DEPOTLEGAL = '"+medicamentDepotLegal+"'");
	...
	public static int incrementationOfStock(String matricule, String codeMedicament, int qteStock) {
		String request = "update STOCK set QTESTOCK = QTESTOCK + '"+qteStock+"' where MATRICULE = '"+matricule+"' AND MED_DEPOTLEGAL = '"+codeMedicament+"'";
